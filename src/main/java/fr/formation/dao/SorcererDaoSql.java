...
public class SorcererDaoSql extends AbstractDaoSql implements ISorcererDao {
	...
	public ArrayList<Sorcerer> findAll() {
		...
		try {
			...
			// S�lectionner tous les sorciers
			ResultSet rs = this
	...
	public Sorcerer add(Sorcerer entity) {
		try {
			...
			while (rs.next()) {
				...
				// D�finir l'id du sorcier et ses attributs
			...
			System.out.println("Sorcier " + entity.getName() + " vient de na�tre !");
			return entity;
	...
	public Sorcerer update(Sorcerer entity) {
		...
		try {
			...
			System.out.println("Sorcier modifi� !");
			return entity;
	...
	private Sorcerer map(ResultSet rs) {
		...
		try {
			...
			// D�finir les attributs du personnage
			int id = rs.getInt("PER_ID");
			...
			// D�finir les attributs du sorcier
			...
			// D�finir les attributs restants du personnage
			sorcerer.setId(id);
